...
func TestRowToStructByTag(t *testing.T) {
	type person struct {
		First string `pgx:"first_name"`
		Last  string `pgx:"last_name"`
		Age   int32  `pgx:"age"`
	}

	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *pgx.Conn) {
		rows, _ := conn.Query(ctx, `select 'Joe' as first_name, 'Smith' as last_name, n as age from generate_series(0, 9) n`)
		slice, err := pgx.CollectRows(rows, pgx.RowToStructByTag[person])
		require.NoError(t, err)

		assert.Len(t, slice, 10)
		for i := range slice {
			assert.Equal(t, "Joe", slice[i].First)
...
func TestRowToStructByTagDefaultSnakeCase(t *testing.T) {
	type person struct {
		FirstName string
		Last      string `pgx:"last_name"`
		Age       int32  `pgx:"age"`
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *pgx.Conn) {
		...
		assert.Len(t, slice, 10)
		for i := range slice {
			assert.Equal(t, "Joe", slice[i].FirstName)
			assert.Equal(t, "Smith", slice[i].Last)
			assert.EqualValues(t, i, slice[i].Age)
		}
	})
...
func TestRowToStructByTagIgnore(t *testing.T) {
	type person struct {
		First string `pgx:"-"`
		Last  string `pgx:"last_name"`
		Age   int32  `pgx:"age"`
...
func TestRowToStructByTagEmbeddedStruct(t *testing.T) {
	type Name struct {
		First string `pgx:"first_name"`
		Last  string `pgx:"last_name"`
	}
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *pgx.Conn) {
		...
		assert.Len(t, slice, 10)
		for i := range slice {
			assert.Equal(t, "Joe", slice[i].Name.First)
			assert.Equal(t, "Smith", slice[i].Name.Last)
			assert.EqualValues(t, i, slice[i].Age)
		}
	})
...
func TestRowToStructByTagMultipleEmbeddedStruct(t *testing.T) {
	type Name struct {
		First string `pgx:"first_name"`
		Last  string `pgx:"last_name"`
	}
	...
	defaultConnTestRunner.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *pgx.Conn) {
		rows, _ := conn.Query(ctx, `select 'Joe' as first_name, 'Smith' as last_name, 'New York' as city`)
		slice, err := pgx.CollectRows(rows, pgx.RowToStructByTag[person])
		require.NoError(t, err)

		assert.Len(t, slice, 1)
		assert.Equal(t, "Joe", slice[0].Name.First)
		assert.Equal(t, "Smith", slice[0].Name.Last)
		assert.EqualValues(t, "New York", slice[0].Address.City)
	})
}

func ExampleRowToStructByTag() {
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
...
`)
	...
	type person struct {
		FirstName string `pgx:"first_name"`
		LastName  string
		Age       int32
	}

	rows, _ := conn.Query(ctx, "select * from people where age < $1 order by age desc", 13)
	people, err := pgx.CollectRows(rows, pgx.RowToStructByTag[person])
	if err != nil {
		fmt.Printf("CollectRows error: %v", err)
		return
	}
